...
# simpele grafiek over hoe sterk de sexe reageren.
```
...
# mannen reageren (gem) meer op de muziek.
# vrouwen zitten vaker dicht bij het gemmiddelde, bij mannen zijn er # meer uizonderingen/uitschieters.
```
...
ggplot(data = file, mapping = aes(x = tijdstip, y = bpm_diff)) +
geom_boxplot()

# mensen reageren heftiger na tijdstip 10-12, mogelijk door recente # consumptie (lunch).
```
...
ggplot(data = file, mapping = aes(x = caffeine, y = bpm_diff)) +
geom_boxplot()

# False = gesport / gesport en caffeine / hartproblemen
# True = caffeine
# NA = NA bij vragenlijst
